        ...
        <AnimatePresence>
          {advanced && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
              <div className="grid grid-cols-2 gap-4 rounded-xl border border-border bg-bg-card p-4 text-xs sm:grid-cols-4">
                {(['text', 'lexical', 'visual', 'ocr'] as const).map((k) => (
                  <label key={k} className="space-y-1">
                    <div className="flex justify-between capitalize text-fg-muted">
                      <span>{k === 'text' ? 'speech' : k === 'ocr' ? 'on-screen' : k}</span>
                      <span className="font-mono">{(weights[k] ?? { text: 1, lexical: 0.8, visual: 0.8, ocr: 0.6 }[k]).toFixed(1)}</span>
                    </div>
                    <input
                      type="range" min={0} max={2} step={0.1}
                      value={weights[k] ?? { text: 1, lexical: 0.8, visual: 0.8, ocr: 0.6 }[k]}
