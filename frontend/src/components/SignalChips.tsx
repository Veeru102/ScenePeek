import { Clapperboard, Eye, Image, Mic, ScanText, Sparkles, Type } from 'lucide-react'
...
const meta: Record<string, { label: string; tone: 'speech' | 'visual' | 'ocr' | 'lexical' | 'accent'; icon: typeof Mic }> = {
  ...
  temporal: { label: 'Motion', tone: 'visual', icon: Clapperboard },
...
export function SignalChips({ signals, compact = false }: { signals: Record<string, number>; compact?: boolean }) {
  const order = ['text', 'lexical', 'visual', 'temporal', 'caption', 'ocr', 'rerank']
  return (
